...
struct Point3D {
    ...
    CoordType y;
    CoordType z;
    // Overloading equals operator to manage hash collision
    bool operator==(const Point3D& other) const {
        return this->x == other.x && this->y == other.y && this->z == other.z;
    }
};
// Making hasher to insert type variables into unordered set
struct Hasher {
    size_t operator ()(const Point3D& point) const {
        // Taking big simple number as a coefficient
        size_t simpleNumber = 68718952447;
        // Using STL hashers for STL types
        hash<CoordType> coordinateHasher;
        // Returning quadratic polynomial o coefficient and hashes of coordinates
        return simpleNumber * simpleNumber * coordinateHasher(point.x) +
               simpleNumber * coordinateHasher(point.y) +
               coordinateHasher(point.z);
    }
};
