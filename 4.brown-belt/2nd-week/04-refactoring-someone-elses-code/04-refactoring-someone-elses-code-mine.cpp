...
// It's no harm to use std namespace globally in cpp source files
using namespace std;

Json::Document XmlToJson(const Xml::Document& doc) {
    // Setting main namespace in scope by number of occurrences of prefix
    using namespace Json;
    vector<Node> result;
    // Using prefix with methods not from selected namespace
    for (const Xml::Node& child : doc.GetRoot().Children()) {
        result.emplace_back(Node(map<string, Node>{
                {"category", Node(child.AttributeValue<string>("category"))},
                { "amount", Node(child.AttributeValue<int>("amount")) }
        }));
    }
...
Xml::Document JsonToXml(const Json::Document& doc, std::string root_name) {
    ...
    for (const auto& node : doc.GetRoot().AsArray()) {
        root.AddChild(Node("spend",{
                {"category",node.AsMap().at("category").AsString()},
                {"amount",to_string(node.AsMap().at("amount").AsInt())}}));
    }

    return Document(move(root));
}
// Tests, that refactored functions should pass
void TestXmlToJson() {
    Xml::Node root("july", {});
