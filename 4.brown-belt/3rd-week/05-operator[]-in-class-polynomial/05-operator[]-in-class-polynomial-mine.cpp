...
// Actual class to modify
template<typename T>
class Polynomial {
private:
    // Inner class, which we return at bracket operator
    struct DegreeWrapper {
    public:
        DegreeWrapper(vector<T>& data, size_t pos) : data(data), pos(pos) {}
        // If Polynomial element used as lvalue, checking size and resizing before assigning, if needed
        DegreeWrapper& operator =(const T& other) {
            if (data.size() < pos && other != 0)
                data.resize(pos + 1, 0);
            data[pos] = other;
            deleteLeadingZeroes();
        ...
        // If Polynomial element used as rvalue, checking size and returning 0 if position bigger than size
        DegreeWrapper& operator =(T& other) const {
            if (data.size() < pos)
                other = data[pos];
            else
                other = 0;
        ...
        // Operators, needed for unit test framework
        bool operator ==(const T& other) const {
            if (data.size() <= pos)
                return 0 == other;
            else
                return data[pos] == other;
        }
        ...
        friend ostream& operator <<(ostream& output, DegreeWrapper item) {
            if (item.data.size() < item.pos)
                output << 0;
            else
                output << item.data[item.pos];

            return output;
        }
        // Saving reference to original vector and needed position
    private:
        vector<T>& data;
        size_t pos;
        // Method, needed for deleting all meaningless zeroes in polynomial after changing values in it
...
public:
    ...
    // Actual implemented operator
    DegreeWrapper operator [](size_t degree) {
        return DegreeWrapper(coeffs_, degree);
    }
...
template <typename T>
ostream& operator <<(std::ostream& out, const Polynomial<T>& p) {
    bool printed = false;
